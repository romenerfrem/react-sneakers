  ...
  const onAddToCart = async (obj) => {
    try {
      ...
      if (findItem) {
        ...
        await axios.delete(`https://620e05f3585fbc3359d41fe5.mockapi.io/cart/${findItem.id}`);
      } else {
        setCartItems((prev) => [...prev, obj]);
        const { data } = await axios.post('https://620e05f3585fbc3359d41fe5.mockapi.io/cart', obj);
        setCartItems((prev) =>
          prev.map((item) => {
    ...
    try {
      if (favorites.find((favObj) => Number(favObj.id) === Number(obj.id))) {
        ...
        const { data } = await axios.post(
          'https://620e05f3585fbc3359d41fe5.mockapi.io/favorites',
          obj,
        );
